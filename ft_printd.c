...
/*   Updated: 2020/01/21 20:35:43 by hmoumani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libftprintf.h"

void		ft_first_part(int len, int *width, char **s, int i)
{
	*width = (g_flags.prec > len) ? ft_abs(g_flags.width) - \
	...
	else if (g_flags.width > len)
	{
		...
		ft_abs(g_flags.prec) && g_flags.fill == '0')
		{
			(*s)++;
			ft_putchar_fd('-', 1);
			width--;
...
void		ft_print_prec(int len, char *s, int i)
{
	int		width;
	int		opt;

	opt = 0;
	ft_first_part(len, &width, &s, 0);
	if (s[0] == '-')
	{
		s++;
		opt = 1;
		ft_putchar_fd('-', 1);
		if (ft_abs(g_flags.width) <= ft_abs(g_flags.prec))
			width--;
		i--;
	}
	while (++i < g_flags.prec - len + 1)
		g_size += write(1, "0", 1);
	ft_putstr_fd(s, 1);
	(len < g_flags.prec && g_flags.width < 0 && g_flags.prec > 0 \
	&& ft_abs(g_flags.width) > g_flags.prec) ? width = ft_abs(g_flags.width) \
	- ft_getmax(g_flags.prec, len) + 1 : 1;
	(opt && ft_abs(g_flags.width) > ft_abs(g_flags.prec)) ? width-- : 1;
	if (g_flags.width * -1 > len + i)
		while (--width > 0 && g_flags.width < 0)
			g_size += write(1, " ", 1);
}

int			ft_printd_f(int size, int n, char *s, int i)
{
	int len;

	if (size > 0 && g_flags.width && (len = ft_strlen(s)))
	{
		(n < 0 && g_flags.fill == '0' && !g_flags.haspoint) \
		? g_size += write(1, "-", 1) : 1;
		(n < 0 && g_flags.fill == '0' && !g_flags.haspoint) ? n = n - 1 : 1;
		(n < 0 && g_flags.fill == '0' && !g_flags.haspoint) ? s++ : s;
	...
	if (n == 0 && g_flags.prec == 0 && g_flags.haspoint && g_flags.width)
		ft_putchar_fd(' ', 1);
	else if (n == 0 && g_flags.prec == 0 && g_flags.haspoint \
	&& !g_flags.width)
		return (1);
	else
		ft_putstr_fd(s, 1);
